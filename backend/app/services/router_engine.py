...
Department Token Governance, PII Guardrail Sanitizer, Chaos Outage Simulation, and Circuit Breaker resilience.
"""
import logging
import re
import time
...
class RouterEngine:
    """
    Smart LLM Router Engine managing provider selection, PII guardrails,
    Upstash Redis semantic caching, department token governance, and circuit breaker resilience.
    """
    ...
    def select_optimal_provider(self, request: GatewayChatRequest, simulate_outage: bool = False) -> Tuple[str, str]:
        ...
        if request.model:
            ...
            elif "openrouter" in model_lower or "/" in request.model:
    ...
    async def _call_groq_api(self, request: GatewayChatRequest, model: str, simulate_outage: bool = False) -> Dict[str, Any]:
        ...
        """
        if simulate_outage:
            logger.warning("⚠️ Groq Circuit Breaker Triggered (503) -> Fallback to OpenRouter")
    ...
    async def _call_openrouter_api(self, request: GatewayChatRequest, model: str) -> Dict[str, Any]:
        """
        Execute completion request against OpenRouter API via httpx.AsyncClient.
        """
        ...
        headers = {
            "Authorization": f"Bearer {api_key}",
            "HTTP-Referer": "https://nexusgateway.io",
    ...
    async def process_chat_completion(
        ...
        """
        Full Execution Pipeline with Outage Chaos Simulation and Resilience Fallback.
        """
        start_time = time.time()
        department = request.department or "general"
        simulate_groq_outage = header_outage_flag or (request.simulate_outage == "groq")

        # --- Step 1: Department Budget Governance Check ---
        ...
        # --- Step 2: PII Guardrail Scrubbing ---
        ...
        # --- Step 3: Semantic Cache Lookup (Skip cache if chaos testing outage) ---
        ...
        # --- Step 4: Provider Selection & Fallback Execution ---
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Groq Circuit Breaker Triggered (503) -> Fallback to OpenRouter (Error: {primary_exc})"
            )
            ...
            try:
                logger.info(f"🔄 Executing FALLBACK request to '{fallback_provider}' ({fallback_model})...")
                provider_used, raw_response = await self.execute_provider(
                ...
                resilience_engine.record_failure(fallback_provider)
                ...
                cost_info = calculate_tokens_and_cost(
                    ...
                    "nexus-resilience-fallback",
                    "llama-3.3-70b-versatile"
                )

                response_obj = GatewayChatResponse(
                    id=f"chatcmpl-fallback-{uuid.uuid4().hex[:8]}",
                    provider_used="nexus-resilience-fallback",
                    model_used="llama-3.3-70b-versatile",
                    content=(
                        f"[NexusGateway Autopilot Fallback Response] Primary '{primary_provider}' and "
                        f"fallback '{fallback_provider}' providers were unavailable or unconfigured. "
                        f"Request captured gracefully by resilience layer for department '{department}'."
                    ),
        ...
        # --- Step 6 & 7: Record Usage & Store Cache ---
        record_department_usage(
            department,
            response_obj.total_tokens,
            response_obj.estimated_cost_usd,
